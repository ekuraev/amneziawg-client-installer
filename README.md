...
Обычный WireGuard легко опознаётся системами DPI, и в сетях с геоблокировками
и цензурой такие туннели режут или замедляют. AmneziaWG сохраняет протокол и
производительность WireGuard, но маскирует трафик, так что соединение
переживает блокировки. Серверных установщиков много, а поставить клиент на
Linux-машину без графики до сих пор означало руками собирать модуль ядра,
утилиты и разбираться с systemd. Этот скрипт сводит всё к одной команде.

Где это полезно:

- **Рабочие и домашние сети.** Постоянный канал до собственного сервера или
  офиса с машины, которая должна быть в сети всегда: NAS, домашний сервер,
  мини-ПК.
- **Edge-устройства и IoT-шлюзы.** Raspberry Pi и аналоги в удалённых точках,
  которым нужен устойчивый доступ к облаку или к центральной инфраструктуре
  через ненадёжные и фильтруемые каналы.
- **Серверы и VPS.** Доступ к заблокированным API и репозиториям, связь между
  площадками в разных юрисдикциях, обход региональных ограничений провайдера.
- **Парк устройств.** Один и тот же однострочник с готовым конфигом
  разворачивается на десятках машин через SSH или систему управления
  конфигурацией, а `--config-only` обновляет ключи и параметры за секунды.
