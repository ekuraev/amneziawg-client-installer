...
Один скрипт, который превращает Raspberry Pi (или любой Debian/Ubuntu с systemd)
в **клиент** AmneziaWG. Никакой серверной части: только модуль, утилиты, ваш
...
- Raspberry Pi OS (Bookworm/Trixie, 32 или 64 бит), Debian 12+, Ubuntu 22.04+.
  Другие apt-системы с systemd, скорее всего, тоже подойдут.
- Доступ в интернет с Pi (GitHub, go.dev, репозитории apt).
...
Скопируйте клиентский конфиг на Pi (например, `scp client.conf pi@raspberrypi.local:~/`),
затем на самой Pi:
...
`sudo bash -s -- ~/client.conf --exclude-lan`.
...
На Raspberry Pi 4 полная установка занимает 5–10 минут, большая часть уходит
на сборку модуля ядра.

## Параметры

```
sudo ./install-awg-client.sh <client.conf> [--iface NAME] [--exclude-lan[=CIDR,...]] [--userspace] [--no-start]
```
...
запуска весь трафик Pi, включая вашу SSH-сессию, пойдёт через VPN. Скрипт
...
сам добавляет правило `suppress_prefixlength 0`, поэтому подсеть, к которой Pi
...
**DNS.** Строка `DNS =` в конфиге применяется через `resolvconf`. На Raspberry
Pi OS его обычно нет, и скрипт поставит `openresolv`. Если активен
...
`amneziawg-go` работает везде, но на Pi 4 ограничивает скорость примерно
100–200 Мбит/с. Какой вариант установлен, видно в итоговой сводке скрипта.
Если в системе есть и модуль, и `amneziawg-go`, `awg-quick` использует модуль.

## Обновление

Просто запустите скрипт ещё раз с тем же конфигом. Он подтянет свежие теги,
пересоберёт компоненты, сохранит прежний конфиг как `awg0.conf.bak.<дата>`
(если новый отличается) и перезапустит туннель.
...
проверяются на самой Raspberry Pi.
